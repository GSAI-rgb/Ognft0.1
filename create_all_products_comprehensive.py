...
def scan_all_products():
            ...
            if design_folder.is_dir():
                ...
                for item in design_folder.iterdir():
                    if item.is_dir() and item.name.lower() in ['blue', 'black', 'grey', 'red', 'navy', 'white', 'green', 'brown', 'purple']:
                        ...
                        for img in color_images:
                            images.append(f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}")
                ...
                if back_dir.exists():
                    ...
                    for img in back_images:
                        images.insert(0, f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}")  # Insert at beginning for priority
                        ...
                        if subfolder.is_dir():
                            ...
                            for img in sub_images:
                                if 'back' in subfolder.name.lower():
                                    images.insert(0, f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}")
                                else:
                                    images.append(f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}")
    ...
    if headband_dir.exists():
        ...
        if headband_images:
            images = [f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}" for img in headband_images]
