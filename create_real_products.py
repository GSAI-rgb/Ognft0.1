...
def create_real_products():
    ...
    for design, config in teeshirt_designs.items():
        ...
        if config["colors"]:
            for color in config["colors"]:
                ...
                if color_dir.exists():
                    ...
                    images.extend([f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}" for img in color_images])
        ...
        if back_dir.exists():
            ...
            back_images = [f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}" for img in back_imgs]
        
        if front_dir.exists():
            front_imgs = list(front_dir.glob("*.jpg")) + list(front_dir.glob("*.jpeg"))
            front_images = [f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}" for img in front_imgs]
    ...
    if hoodie_dir.exists():
        ...
        for hoodie_folder in hoodie_dir.iterdir():
            if hoodie_folder.is_dir():
                ...
                if direct_imgs:
                    direct_images = [f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}" for img in direct_imgs]
    ...
    if poster_dir.exists():
        for poster_folder in poster_dir.iterdir():
            if poster_folder.is_dir():
                ...
                if poster_images:
                    images = [f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}" for img in poster_images]
        ...
        for shirt_folder in shirts_dir.iterdir():
            if shirt_folder.is_dir():
                ...
                if front_dir.exists():
                    ...
                    if shirt_images:
                        images = [f"https://shopify-debugger.preview.emergentagent.com/products/{img.relative_to(products_dir)}" for img in shirt_images]
